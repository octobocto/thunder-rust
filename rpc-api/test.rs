...
fn check_schema() -> anyhow::Result<()> {
    let mut schema = crate::open_api::RpcDoc::openapi();
    schema.merge(crate::node::PrivateRpcDoc::openapi());
    schema.merge(crate::node::RpcDoc::openapi());
    schema.merge(crate::wallet::RpcDoc::openapi());
    ...
    for ref_loc in &component_ref_locations {
        match ref_loc.strip_prefix("#/components/schemas/") {
            Some(loc) if component_schemas.contains(loc) => (),
            _ => missing.push(*ref_loc),
        }
    }
    anyhow::ensure!(missing.is_empty(), "Missing schemas: {missing:?}");
    Ok(())
}
